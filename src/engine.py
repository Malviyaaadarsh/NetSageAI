...
def call_gemini(system_prompt: str, user_payload: str) -> Dict[str, object]:
	...
	if not parts:
		raise ValueError(f"Gemini returned empty content parts: {body}")

	content = "\n".join(_safe_text(part.get("text")) for part in parts)
...
def diagnose_case(case: Mapping[str, object], prompt_path: Path) -> Dict[str, object]:
	...
	try:
		llm_result = call_gemini(prompt, _build_user_payload(case, checker_result))
