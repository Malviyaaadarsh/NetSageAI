...
def _bootstrap_gemini_env_from_secrets() -> None:
	...
	if gemini_model and not os.getenv("GEMINI_MODEL"):
...
st.title("NetSage AI - Packet Tracer Troubleshooting Assistant")
st.caption("Hybrid deterministic + LLM diagnostics with mandatory Human-in-the-Loop review")

_bootstrap_gemini_env_from_secrets()
...
with st.sidebar:
	...
	gemini_key_present = bool(os.getenv("GEMINI_API_KEY") or st.secrets.get("GEMINI_API_KEY", None))
	st.write("Gemini key status: " + ("Configured" if gemini_key_present else "Not configured"))
	st.write("Gemini model: " + str(os.getenv("GEMINI_MODEL", st.secrets.get("GEMINI_MODEL", "gemini-1.5-flash"))))
...
if st.button("Run Hybrid Diagnosis", type="primary"):
	with st.spinner("Running checker and Gemini diagnosis..."):
...
if "diagnosis" in st.session_state:
	...
	with right:
		...
		if note:
			st.warning(f"Gemini fallback activated: {note}")
